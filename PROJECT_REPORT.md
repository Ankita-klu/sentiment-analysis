...
$$\text{similarity}(\text{love}, \text{amazing}) = \cos(\vec{v}_{\text{love}}, \vec{v}_{\text{amazing}}) \approx 0.82$$

**Document Representation:** Average word vectors:
$$\vec{v}_{\text{tweet}} = \frac{1}{|tweet|} \sum_{w \in tweet} \vec{v}_w$$
...
2. **Consistency:** Same approach used in sklearn baselines for fair comparison
3. **Interpretability:** Can see exactly which words contribute to classification

**Trade-off Accepted:**
We sacrifice 20-25% accuracy (embeddings would likely improve to 75-80% range based on literature) in exchange for implementation clarity and understanding.
...
$$\text{relative\_error} = \frac{||\nabla_{\text{numerical}} - \nabla_{\text{analytical}}||_2}{||\nabla_{\text{numerical}}||_2 + ||\nabla_{\text{analytical}}||_2 + 10^{-8}}$$
...
- Gradient checking: approximately 10,000 times slower than normal backprop
...
2. **Data bugs:** If preprocessing is broken, gradients are still mathematically correct
3. **Optimization bugs:** If learning rate is too high/low, gradients are correct but training fails
4. **Initialization bugs:** If weights start at bad values, gradients are correct but convergence is poor
...
- **Training Loss:** Decreases smoothly from 1.37 (epoch 0) to 1.07 (epoch 50)
- **Validation Loss:** Decreases smoothly from 1.37 (epoch 0) to 1.04 (epoch 50)
...
|-------|-------------|----------|------------|
| **SVM (RBF kernel)** | **0.8224** | 82.24% | High |
| **Logistic Regression** | **0.7721** | 77.21% | Low |
| **Naive Bayes** | **0.7187** | 71.87% | Low |
| **MLP (Custom)** | **0.5308** | 56.46% | Medium |
...
- Reduced memory footprint from 288 MB to approximately 0.5 MB per batch
...
- Converges in 50 epochs with these optimizations (based on our training run)
...
[PASS] **Learning rate decay improves final accuracy** (2-3 percentage points improvement observed)  
...
2. [PASS] 3-layer MLP with ReLU and softmax activations
3. [PASS] Backpropagation with analytical gradient computation
4. [PASS] SGD with momentum and learning rate decay
5. [PASS] L2 regularization for overfitting prevention
6. [PASS] Gradient checking for mathematical validation
7. [PASS] Interactive Streamlit dashboard for model analysis
