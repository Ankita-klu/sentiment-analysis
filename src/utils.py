...
def confusion_matrix(y_pred, y_true, n_classes=4):
    ...
    for t, p in zip(y_true, y_pred):
        cm[int(t), int(p)] += 1
    return cm
# File: src/utils.py (add at the end)
...
def gradient_check(model, X_sample, y_sample, epsilon=1e-7):
    ...
    Returns: difference ratio (should be < 1e-5)
    """
    import copy
    
    # Forward & backward to get analytical gradients
    y_pred = model.forward(X_sample)
    model.backward(y_sample, lr=0.001)  # Don't actually update
    ...
    for layer_idx in range(len(model.W)):
        ...
        numerical_db.append(db_layer)  # Simplified: not checking bias
